...
function append_book_items(book_list,admin=false) {
  ...
  for (book of book_list) {
    ...
    <li class="${admin ? `admin-list-items`: `list-items`}">
      ...
      <span>${book[2]}</span>
      <span>${book[3]}</span>
      ${admin ? `<span>${book[4]}</span> <span> <form> <input class= "restock-${book[0]}" type = "number" required placeholder="Enter Quantity"> <button onclick="handleRestock(event, '${book[4]}', '${book[0]}')">Restock</button> </form> </span>`
...
function handleAdminAccess(event) {
  ...
  eel.check_pass(password)().then(function(res) {
    res ? window.location.href = "/admin.html" : alert("WRONG PASSWORD ENTERED!");
  })
...
function handleIssueBook(event) {
  ...
  eel.issue_book(name, book)().then(res => console.log(res))
...
function handleAddBook(event) {
  ...
  eel.add_book(book_name,genre,fine,quantity)().then(res => console.log(res))
}

function handleRestock(event, quantity, bookId) {
  event.preventDefault()
  let newQuantity = Number(document.querySelector(`.restock-${bookId}`).value);
  eel.restock(newQuantity + Number(quantity), bookId)().then(res => console.log(res))
}
